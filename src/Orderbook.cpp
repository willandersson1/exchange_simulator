...
void OrderBook::submitOrder(Order &O)
{
    ...
    map<Price, OrderBookEntry> &entriesToUse = isBuy ? sell_entries : buy_entries;
...
{
    map<Price, OrderBookEntry> &entriesAddingTo = O.direction == OrderDirection::BUY ? buy_entries : sell_entries;
